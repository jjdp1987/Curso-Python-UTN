# Definir una lista por asignación que almacene los nombres de los primeros cuatro meses de año.
# Mostrar el primer y último elemento de la lista solamente.

meses=["enero", "febrero", "marzo", "abril"]
print(meses[0]) # se muestra enero
print(meses[3]) # se muestra abril

#Como queremos imprimir solo el primer y último elemento de la lista indicamos entre corchetes la
#posición de la lista del cual queremos rescatar el valor.
#Si llamamos a print y pasamos solo el nombre de la lista luego se nos muestra todos los elementos:
#print(meses) # se muestra ["enero", "febrero", "marzo", "abril"]